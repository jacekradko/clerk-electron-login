...
const FormLabel = React.forwardRef<React.ElementRef<typeof LabelPrimitive.Root>, React.ComponentPropsWithoutRef<typeof LabelPrimitive.Root>>(
	({ className, ...props }, ref) => {
		...
		return (
			<Label ref={ref} className={`text-muted-foreground ${cn(error && `text-destructive-foreground`, className)}`} htmlFor={formItemId} {...props} />
		)
	}
)
...
const FormMessage = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(({ className, children, ...props }, ref) => {
	...
	return (
		<p ref={ref} id={formMessageId} className={cn('text-sm font-normal text-destructive-foreground mt-1', className)} {...props}>
			{body}
		</p>
	)
})
